...
Shader "Hidden/WaterCausticsModules/TexGenShader" {
    ...
    SubShader {
        ...
        Pass {
			...
			#pragma multi_compile_local _ _FILTER_LINEAR
			...
			half4 _WCM_TG_Offset;
			half2 _WCM_TG_OffsetColor;
            ...
            v2f vert(vIn v) {
                ...
                o.pos = float4(v.vertex.xy * 2 - 1, 0, 1);
                    ...
                    o.uvG[3] = uv1.zw;
                    #if defined(_USE_RGB)
						half2 ofc = _WCM_TG_OffsetColor.xy;
                        o.uvR[0] = uv0.xy - ofc;
                        o.uvR[1] = uv0.zw - ofc;
                        o.uvR[2] = uv1.xy - ofc;
                        o.uvR[3] = uv1.zw - ofc;
			...
			half _WCM_TG_Gamma;
			half _WCM_TG_Brightness;

			half sampleTex(const half2 uv, const int channel) {
				#if defined(_FILTER_LINEAR)
					return _MainTex.Sample(sampler_LinearRepeat, uv)[channel];
				#else
					return _MainTex.Sample(sampler_PointRepeat, uv)[channel];
				#endif
			}

            #if defined(_SAMPLE4)
                half sample(const half2 uv[4], const int channel) {
					half c0 = sampleTex(uv[0], channel);
					half c1 = sampleTex(uv[1], channel);
					half c2 = sampleTex(uv[2], channel);
					half c3 = sampleTex(uv[3], channel);
                    return (c0 + c1 + c2 + c3) * 0.25;
                }
            #else // _SAMPLE1
                half sample(const half2 uv[1], const int channel) {
					return sampleTex(uv[0], channel);
                }
            #endif
            ...
            half4 frag(v2f i) : COLOR {
                half4 c;
                    ...
                    #if defined(_BRIGHTNESS_ADJ)
						s = pow(s, _WCM_TG_Gamma) * _WCM_TG_Brightness;
                    #endif
                    c.rgba = s.rrrr;
                #endif
                return c;

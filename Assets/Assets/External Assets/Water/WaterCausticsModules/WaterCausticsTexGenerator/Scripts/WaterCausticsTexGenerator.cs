...
#pragma warning disable IDE0044
        ...
        [SerializeField] private bool m_animateInEditMode = false;
...
#pragma warning restore IDE0044
        ...
        private void OnEnable () {
            if (_cmd == null)
                _cmd = new CommandBuffer { name = "WaterCausticsTexGen" };
#if UNITY_2021_3_OR_NEWER
            _localCSKeywords.Clear ();
            _localMatKeywords.Clear ();
#endif
#if UNITY_EDITOR
            EditorApplication.update -= constantUpdate;
            EditorApplication.update += constantUpdate;
            CheckRenderTex ();
#endif
        }
        ...
        private class pID {
            ...
            readonly internal static int _BufMeshRW = Shader.PropertyToID ("_BufMeshRW");
            readonly internal static int _VertsCnt = Shader.PropertyToID ("_VertsCnt");
            readonly internal static int _LightDirection = Shader.PropertyToID ("_LightDirection");
            readonly internal static int _WCM_TG_Brightness = Shader.PropertyToID ("_WCM_TG_Brightness");
            readonly internal static int _WCM_TG_Gamma = Shader.PropertyToID ("_WCM_TG_Gamma");
            readonly internal static int _WCM_TG_Offset = Shader.PropertyToID ("_WCM_TG_Offset");
            readonly internal static int _WCM_TG_OffsetColor = Shader.PropertyToID ("_WCM_TG_OffsetColor");
            readonly internal static int _TempRT1 = Shader.PropertyToID ("_WCTempRT1");
            readonly internal static int _TempRT2 = Shader.PropertyToID ("_WCkTempRT2");
        }
        ...
        private ComputeShader getComputeShader () {
            ...
            if (m_computeShader == null) {
                ...
                __computeShader = Instantiate (m_computeShader);
        ...
        private int calcVerticesCnt (int res) {
            int wholeWidth = res + (int) (res * m_FillGap) * 2;
        ...
        private void setupMesh () {
            ...
            for (int vi = 0, i = 0; i < bufSz3or1; i++) {
                for (int y = 0; y <= wholeWidth; y++) {
                    for (int x = 0; x <= wholeWidth; x++, vi++) {
                        ...
                        float vZ = i * bufArea + idx + 0.1f;
                        vertices [vi] = new Vector3 (vX * 2f - 1f, vY * 2f - 1f, vZ);
                    }
                }
            }
            int [] indicesAry = new int [wholeWidth * wholeWidth * 6 * bufSz3or1];
            for (int ti = 0, vi = 0, i = 0; i < bufSz3or1; i++, vi += wholeWidth + 1) {
                for (int y = 0; y < wholeWidth; y++, vi++) {
                    for (int x = 0; x < wholeWidth; x++, ti += 6, vi++) {
                        indicesAry [ti] = vi;
                        indicesAry [ti + 2] = indicesAry [ti + 3] = vi + 1;
                        indicesAry [ti + 1] = indicesAry [ti + 4] = vi + wholeWidth + 1;
                        indicesAry [ti + 5] = vi + wholeWidth + 2;
                    }
                }
            }
            if (__mesh == null)
            ...
            __mesh.Clear ();
#if UNITY_2021_3_OR_NEWER
            release (ref _bufMesh);
            int indicesCnt = indicesAry.Length;
            __mesh.indexBufferTarget |= GraphicsBuffer.Target.Raw;
            __mesh.vertexBufferTarget |= GraphicsBuffer.Target.Raw;
            ...
            __mesh.SetIndexBufferParams (indicesCnt, IndexFormat.UInt32);
            __mesh.SetVertexBufferParams (verticesCnt, layout);
            __mesh.SetSubMesh (0, new SubMeshDescriptor (0, indicesCnt), MeshUpdateFlags.DontRecalculateBounds);
            __mesh.SetIndexBufferData (indicesAry, 0, 0, indicesCnt, MeshUpdateFlags.DontRecalculateBounds);
            var vertDataAry = new VertexStruct [verticesCnt];
            for (var i = 0; i < verticesCnt; ++i) {
                vertDataAry [i].pos = Vector2.zero;
                vertDataAry [i].col = Vector3.zero;
                vertDataAry [i].initPos = vertices [i];
                vertDataAry [i].dataIdx = (uint) vertices [i].z;
            }
            __mesh.SetVertexBufferData (vertDataAry, 0, 0, verticesCnt);
            _bufMesh = __mesh.GetVertexBuffer (0);
#else
            __mesh.indexFormat = (verticesCnt >= 65536) ? IndexFormat.UInt32 : IndexFormat.UInt16;
            __mesh.vertices = vertices;
            __mesh.triangles = indicesAry;
#endif
        ...
        // ----------------------------------------------------------- GraphicsBuffer
        private GraphicsBuffer _bufNoise, _bufRefract, _bufMesh;
        ...
        // ----------------------------------------------------------- Update
        private static readonly Dictionary<RenderTexture, WaterCausticsTexGenerator> s_drawer = new Dictionary<RenderTexture, WaterCausticsTexGenerator> ();
        ...
        private void LateUpdate () {
            ...
            float deltaTime = m_unscaledTime ? Time.unscaledDeltaTime : Time.deltaTime;
#if UNITY_EDITOR
            if (!Application.isPlaying && !needAnimInEditMode) deltaTime = 0f;
#endif
        ...
        private void generate (float deltaTime, RenderTexture rt) {
            ...
            setConstantBuffer (_cmd, deltaTime);

            // -- Compute shader
            calcComputeShader (_cmd);

            // -- Draw mesh
            if (m_usePostProcessing) {
                getPPTmpRT (_cmd, pID._TempRT1, rt, useMSAA: true);
                drawMesh (_cmd);
                postProcessing (_cmd, rt);
            } else {
                drawMesh (_cmd, rt);
            }
        ...
        private readonly Vector4 [] _tmpDataAry = new Vector4 [WAVE_MAX_CNT];
        private readonly Vector4 [] _tmpUVAry = new Vector4 [WAVE_MAX_CNT];
        private readonly Vector4 [] _tmpDirAry = new Vector4 [WAVE_MAX_CNT];
        ...
        private void setConstantBuffer (CommandBuffer cmd, float delteTime) {
            ...
            cmd.SetComputeIntParam (cs, pID._WaveCnt, waveIdx);
            cmd.SetComputeVectorArrayParam (cs, pID._WaveData, _tmpDataAry);
            cmd.SetComputeVectorArrayParam (cs, pID._WaveUVShift, _tmpUVAry);
            cmd.SetComputeVectorArrayParam (cs, pID._WaveNoiseDir, _tmpDirAry);

            // Resolution
            int calcRes = (int) m_calcResolution;
            float texel = 1f / (float) calcRes;
            cmd.SetComputeFloatParam (cs, pID._CalcTexel, texel);
            cmd.SetComputeFloatParam (cs, pID._CalcTexelInv, (float) calcRes);
            cmd.SetComputeIntParam (cs, pID._CalcResUI, calcRes);
            cmd.SetComputeIntParam (cs, pID._IdxStride, calcRes * calcRes);
            ...
            cmd.SetComputeVectorParam (cs, pID._LightDir, litDir);

            // Brightness
            cmd.SetComputeFloatParam (cs, pID._Brightness, m_brightness * _lcStyleBright [(int) m_style] * 0.1f);
            cmd.SetComputeFloatParam (cs, pID._Gamma, m_gamma * _lcStyleGamma [(int) m_style]);
            cmd.SetComputeFloatParam (cs, pID._Clamp, m_clamp);

            // Refraction index
            float eta = 1f / m_refractionIndex;
            float chrAb = 1f + m_chromaticAberration;
            Vector3 refractIdx = new Vector3 (eta * chrAb, eta, eta / chrAb);
            cmd.SetComputeVectorParam (cs, pID._Eta, refractIdx);

            // Offset drawing position
            Vector3 refractG = refract (litDir, eta);
            if (m_refractedRay == RefractedRay.Normalize) {
                setCSKeyword (cmd, cs, "EXTEND_RAY", false);
                cmd.SetComputeVectorParam (cs, pID._DrawOffset, -(Vector2) refractG);
            } else {
                setCSKeyword (cmd, cs, "EXTEND_RAY", true);
                cmd.SetComputeVectorParam (cs, pID._DrawOffset, -(Vector2) refractG / refractG.z);
            }
        }

        private void calcComputeShader (CommandBuffer cmd) {
            var cs = getComputeShader ();
            if (cs == null) return;

            setCSKeyword (cmd, cs, _lcStyleStr [(int) Style.StyleA], m_style == Style.StyleA);
            setCSKeyword (cmd, cs, _lcStyleStr [(int) Style.StyleB], m_style == Style.StyleB);
            setCSKeyword (cmd, cs, _lcStyleStr [(int) Style.StyleC], m_style == Style.StyleC);

            int kernel;
            int sc = (int) m_calcResolution / THREAD_SIZE;
            int shift = useChromAbe ? 1 : 0;

            cmd.SetComputeBufferParam (cs, kID.NoiseCS, pID._BufNoiseRW, _bufNoise);
            cmd.DispatchCompute (cs, kID.NoiseCS, sc, sc, 1);

            kernel = kID.RefractCS + shift;
            cmd.SetComputeBufferParam (cs, kernel, pID._BufNoise, _bufNoise);
            cmd.SetComputeBufferParam (cs, kernel, pID._BufRefractRW, _bufRefract);
            cmd.DispatchCompute (cs, kernel, sc, sc, 1);
...
#if UNITY_2021_3_OR_NEWER
            kernel = kID.MeshCS;
            cmd.SetComputeBufferParam (cs, kernel, pID._BufRefract, _bufRefract);
            cmd.SetComputeBufferParam (cs, kernel, pID._BufMeshRW, _bufMesh);
            cmd.SetComputeIntParam (cs, pID._VertsCnt, _meshVertsCnt);
            cmd.DispatchCompute (cs, kernel, Mathf.CeilToInt ((float) _meshVertsCnt / THREAD_SIZE), 1, 1);
#endif

            // // ---- for Debug
            // cmd.SetComputeBufferParam (cs, kID.ColorCS + 2, pID._BufNoise, _bufNoise);
            // cmd.SetComputeBufferParam (cs, kID.ColorCS + 2, pID._BufRefractRW, _bufRefract);
            // cmd.DispatchCompute (cs, kID.ColorCS + 2, sc, sc, 1);
            // // ----
        }

#if UNITY_2021_3_OR_NEWER
        readonly Dictionary<string, LocalKeyword> _localCSKeywords = new ();
        readonly Dictionary<string, LocalKeyword> _localMatKeywords = new ();
#endif
        private void setCSKeyword (CommandBuffer cmd, ComputeShader cs, string keyword, bool isOn) {
#if UNITY_2021_3_OR_NEWER
            if (!_localCSKeywords.ContainsKey (keyword))
                _localCSKeywords.Add (keyword, new LocalKeyword (cs, keyword));
            cmd.SetKeyword (cs, _localCSKeywords [keyword], isOn);
#else
            if (isOn)
        ...
        private void setMatKeyword (CommandBuffer cmd, Material mat, string keyword, bool isOn) {
#if UNITY_2021_3_OR_NEWER
            if (!_localMatKeywords.ContainsKey (keyword))
                _localMatKeywords.Add (keyword, new LocalKeyword (m_shader, keyword));
            cmd.SetKeyword (mat, _localMatKeywords [keyword], isOn);
#else
            if (isOn)
        ...
        private void drawMesh (CommandBuffer cmd, RenderTexture rt = null) {
            var mat = getMat ();
            if (mat == null) return;
            cmd.SetRenderTarget (rt ? (RenderTargetIdentifier) rt : pID._TempRT1, RenderBufferLoadAction.DontCare, RenderBufferStoreAction.Store);
            cmd.ClearRenderTarget (false, true, Color.clear);
            setMatKeyword (cmd, mat, "_USE_RGB", useChromAbe);
#if !UNITY_2021_3_OR_NEWER
            mat.SetBuffer (pID._BufRefract, _bufRefract);
#endif
            cmd.DrawMesh (getMesh (), Matrix4x4.identity, mat, 0, 0);
        }
        ...
        private void getPPTmpRT (CommandBuffer cmd, int rtID, RenderTexture rt, bool useMSAA) {
            var desc = new RenderTextureDescriptor (rt.width, rt.height, rt.graphicsFormat, 0) {
                useDynamicScale = rt.useDynamicScale,
                msaaSamples = useMSAA ? Mathf.Min ((int) m_msaa, _sysMaxMSAA) : 1
            };
            cmd.GetTemporaryRT (rtID, desc, FilterMode.Bilinear);
        }

        private void postProcessing (CommandBuffer cmd, RenderTexture dst) {
            ...
            var mat = getMat ();
            setMatKeyword (cmd, mat, "_USE_RGB", useChromAbe);
            cmd.SetGlobalVector (pID._WCM_TG_OffsetColor, Vector2.zero);
            float colShift = 1f;
            bool useColAdjust = (m_postContrast != 1f || m_postBrightness != 1f || QualitySettings.activeColorSpace != ColorSpace.Linear);
            setMatKeyword (cmd, mat, "_BRIGHTNESS_ADJ", false);

            int buf1 = pID._TempRT1;
            int buf2 = pID._TempRT2;

            void sampling (RenderTargetIdentifier src, RenderTargetIdentifier dst, Vector4 offV, bool useColorShift = false) {
                bool canDiv (float v) { return 0.5f - Mathf.Abs (v - Mathf.Floor (v) - 0.5f) <= 0.001f; }
                bool isPoint = !useColorShift && canDiv (offV.x) && canDiv (offV.y) && canDiv (offV.z) && canDiv (offV.w);
                setMatKeyword (cmd, mat, "_FILTER_LINEAR", !isPoint);
                cmd.SetGlobalVector (pID._WCM_TG_Offset, offV);
                cmd.Blit (src, dst, mat, 1);
            }

            for (int i = 0; i < Mathf.Max (1, iteration); i++) {
                bool isLast = i == iteration - 1;
                if (!isLast) getPPTmpRT (cmd, buf2, dst, useMSAA: false);
                var blitDst = isLast ? (RenderTargetIdentifier) dst : buf2;
                float off = (iteration == 1) ? spread : 0.5f + i * spread;
                colShift += off * 0.25f;
                setMatKeyword (cmd, mat, "_SAMPLE4", off != 0f);
                if (isLast && useColAdjust) {
                    setMatKeyword (cmd, mat, "_BRIGHTNESS_ADJ", true);
                    cmd.SetGlobalFloat (pID._WCM_TG_Gamma, m_postContrast * (QualitySettings.activeColorSpace == ColorSpace.Gamma ? 1f / 2.2f : 1f));
                    cmd.SetGlobalFloat (pID._WCM_TG_Brightness, m_postBrightness * m_postContrast);
                }
                if (isLast && m_colorShift > 0f) {
                    Vector2 offCol = dirToVec (m_colorShiftDir) * (m_colorShift * colShift);
                    setMatKeyword (cmd, mat, "_USE_RGB", true);
                    cmd.SetGlobalVector (pID._WCM_TG_OffsetColor, offCol * 0.01f);
                    sampling (buf1, blitDst, offsetBaseV * off, true);
                } else {
                    sampling (buf1, blitDst, offsetBaseV * off);
                }
                cmd.ReleaseTemporaryRT (buf1);
                (buf1, buf2) = (buf2, buf1);
            }

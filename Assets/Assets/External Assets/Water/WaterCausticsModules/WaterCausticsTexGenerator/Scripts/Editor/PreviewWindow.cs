...
namespace MH.WaterCausticsModules {
    internal class PreviewWindow : EditorWindow {
        ...
        static private Texture _iconUnlock;
        static private Texture _iconLock;
        static private Texture _iconMenu;
        static private Shader _iconShader;

        // --------------------------------------------------------------- Event
        private void OnEnable () {
            if (_iconMenu == null && TexGenModuleRef.findAsset<TexGenModuleRef> (out var asset, out var path)) {
                _iconMenu = asset.m_iconMenu;
                _iconLock = asset.m_iconLock;
                _iconUnlock = asset.m_iconUnlock;
                _iconShader = asset.m_iconShader;
            }
        }
        ...
        private Material matIcon () {
            if (__matIcon == null) {
                if (_iconShader == null)
                    _iconShader = Shader.Find ("Hidden/WaterCausticsModules/Icon");
                if (_iconShader == null) {
                    Debug.LogError ("Shader is null. " + this);
                    Close ();
                } else {
                    __matIcon = new Material (_iconShader);
        ...
        void OnGUI () {
            ...
            if (_iconMenu != null) {
                EditorGUI.DrawPreviewTexture (getIconRect (10f, 10f), _iconMenu, matIcon (), ScaleMode.StretchToFill, 0f, -1f);
                if (isLockable)
                    EditorGUI.DrawPreviewTexture (getIconRect (30f, 10f), isLocked ? _iconLock : _iconUnlock, matIcon (), ScaleMode.StretchToFill, 0f, -1f);
            }

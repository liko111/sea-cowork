    ...
    public class DEMO_CamController : MonoBehaviour {
        ...
        private Vector2 getMouseDelta (float divX, float divY) {
            if (_lastGetPosFrame != Time.frameCount - 1) {
                _preMousePos = DEMO_InputWrapper.mousePosition;
                _lastGetPosFrame = Time.frameCount;
                return Vector2.zero;
            } else {
                Vector3 mousePos = DEMO_InputWrapper.mousePosition;
        ...
        private bool pointerOnGameView => new Rect (0f, 0f, Screen.width, Screen.height).Contains (DEMO_InputWrapper.mousePosition);
        ...
        private void LateUpdate () {
            ...
            bool pressBtn0 = DEMO_InputWrapper.touchCount <= 1 && DEMO_InputWrapper.GetMouseButton (0);
            bool pressBtnMid = DEMO_InputWrapper.GetMouseButton (2);
            bool pressAlt = DEMO_InputWrapper.GetKey (KeyCode.LeftAlt);
            bool pressCtrl = DEMO_InputWrapper.GetKey (KeyCode.LeftCommand) || DEMO_InputWrapper.GetKey (KeyCode.LeftControl);
            bool pressShift = DEMO_InputWrapper.GetKey (KeyCode.LeftShift);
            if (!pressBtn0 && !pressBtnMid) _validDrag = false;
            float inputH = DEMO_InputWrapper.GetHorizontalAxis ();
            float inputV = DEMO_InputWrapper.GetVerticalAxis ();
            ...
            // Dolly
            {
                if (DEMO_InputWrapper.touchCount == 2) {
                    var tch0 = DEMO_InputWrapper.GetTouch (0);
                    var tch1 = DEMO_InputWrapper.GetTouch (1);
                ...
                } else if (DEMO_InputWrapper.GetKey (KeyCode.X)) {
                    dolly (-1f, m_SpeedDollyKey * 0.001f);
                } else if (DEMO_InputWrapper.GetKey (KeyCode.Z)) {
                    dolly (1f, m_SpeedDollyKey * 0.001f);
                } else {
                    float scroll = DEMO_InputWrapper.GetMouseScrollWheel ();

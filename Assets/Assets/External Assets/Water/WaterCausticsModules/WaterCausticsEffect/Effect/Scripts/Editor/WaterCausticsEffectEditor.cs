            ...
            if (expandMainGroup (m_method, true, "System", isPink: currentRenderNotHasFeature)) {
                ...
                if (expandSubGroup (m_useImageMask, true, "Influence Scope")) {
...

                    {
                        ...
                        int sysOpqDrawEvt = (int) WaterCausticsEffect.SYS_OPAQUE_TEX_DRAW_EVENT;
                        int sysOpqExistEvt = (int) WaterCausticsEffect.SYS_OPAQUE_TEX_EXIST_EVENT;
                        string sysOpqDrawName = WaterCausticsEffect.SYS_OPAQUE_TEX_DRAW_EVENT.ToString ();
                        string sysOpqExistName = WaterCausticsEffect.SYS_OPAQUE_TEX_EXIST_EVENT.ToString ();
                        string sysOpqDrawTimingStr = $"{sysOpqDrawName}({sysOpqDrawEvt})";
                        string sysOpqExistTimingStr = $"{sysOpqExistName} ({sysOpqExistEvt})";
                        ...
                        EditorGUILayout.LabelField (new GUIContent ("Draw Timing", $"Specifies the timing of drawing.\n\nTo display this effect on _CameraOpaqueTexture, it must be drawn before {sysOpqDrawTimingStr}."), new GUIContent (isHasDifVal ? "-" : $"{baseTimingName} {(adjTiming < 0 ? "-" : "+")}{Mathf.Abs (adjTiming)} ({Mathf.Clamp (adjusted, 0, 1000)})"));
                        if (isExpanded) {
                            using (new IndentScope (-2f, 4f)) {
                                ...
                                bool isEarly = (adjusted <= sysOpqDrawEvt);
                                string warning = isEarly ? $"To use a value between 0 and 1 in the Multiply Color setting, set it after {sysOpqExistTimingStr}." :
                                    $"To display this effect on _CameraOpaqueTexture, it must be drawn before {sysOpqDrawTimingStr}.";
            ...
            if (expandMainGroup (m_texture, true, "Caustics Effect", isPink: useTextureWarning && m_texture.isExpanded)) {
                        ...
                        if (!m_texture.hasMultipleDifferentValues) {
                            if (useTextureWarning) {
                                ...
                                if (GUILayout.Button ("Search from this Scene", EditorStyles.miniButton, GUILayout.Width (150))) {
                                    ...
                                    var gen = FindObjectsByType<WaterCausticsTexGenerator> ().FirstOrDefault (g => g.renderTexture != null);
#elif UNITY_2021_3_OR_NEWER
                                    var gen = FindObjectsByType<WaterCausticsTexGenerator> (FindObjectsSortMode.None).FirstOrDefault (g => g.renderTexture != null);
#else
                ...
                if (expandSubGroup (m_intensity, true, "Effect")) {
                    ...
                    if (!isEditingMultiObj && (m_multiply.floatValue > 0f && m_multiply.floatValue < 1f) &&
                        ((m_renderEvent.intValue + m_renderEventAdjust.intValue) <= (int) WaterCausticsEffect.SYS_OPAQUE_TEX_DRAW_EVENT)
                    ) {
                        // 描画タイミングが早すぎる場合警告
                        using (new IndentScope (0f, 0f)) {

...
namespace MH.WaterCausticsModules {
    ...
    public class WaterCausticsEffect : MonoBehaviour {
        ...
        static internal readonly RenderPassEvent SYS_OPAQUE_TEX_DRAW_EVENT = RenderPassEvent.AfterRenderingSkybox; // URP OpaqueTex描画タイミング
        static internal readonly RenderPassEvent SYS_OPAQUE_TEX_EXIST_EVENT = RenderPassEvent.BeforeRenderingTransparents; // URP OpaqueTexが使用できるようになるタイミング
        static internal readonly RenderPassEvent RENDER_EVENT = SYS_OPAQUE_TEX_DRAW_EVENT; // EachMesh描画タイミング
...
#pragma warning restore IDE0044
        ...
        private bool existOpaqueTex => eventAdjusted >= SYS_OPAQUE_TEX_EXIST_EVENT;
        ...
        private Vector2 calcColorShiftVec () => 0.01f * m_colorShift * dirToVec (m_colorShiftDir);
        ...
        private void enqueuePass (ScriptableRenderer renderer, Camera cam) {
            ...
            if (m_method == Method.EachMesh) {
                ...
                _eachMeshPass ??= new WCEEachMeshPass ();
...
#if UNITY_6000_0_OR_NEWER
        private class PassData { internal RendererListHandle rendererListHdl; }
#endif

        internal class WCEAtOncePass : ScriptableRenderPass {
            private ShaderTagId _shaderTagId;
            private static readonly HashSet<RenderPassEvent> s_hashSet = new HashSet<RenderPassEvent> ();
            ...
            private void setRenderEvt (RenderPassEvent v) {
                if (__evt != v) {
                    ...
                    _queueRange = new RenderQueueRange ((int) v, (int) v);
                }
            }
            ...
            private bool _useOpaqueTex, _useNormalTex;
            internal void Setup (RenderPassEvent evt, bool useNormalTex, bool useOpaqueTex) {
                ...
                if (useNormalTex) request |= ScriptableRenderPassInput.Normal;
                if (useOpaqueTex) request |= ScriptableRenderPassInput.Color;
            ...
            public override void RecordRenderGraph (RenderGraph renderGraph, ContextContainer frameData) {
                ...
                using (var builder = renderGraph.AddRasterRenderPass<PassData> (passName, out var passData, profilingSampler)) {
                    ...
                    builder.UseTexture (resourceDt.activeDepthTexture, AccessFlags.Read);
                    // --
                    registerReadTex (builder, resourceDt.mainShadowsTexture);
                    registerReadTex (builder, resourceDt.additionalShadowsTexture);
                    // --
                    registerReadTex (builder, resourceDt.cameraDepthTexture);
                    if (_useNormalTex) registerReadTex (builder, resourceDt.cameraNormalsTexture);
                    if (_useOpaqueTex) registerReadTex (builder, resourceDt.cameraOpaqueTexture);
                    // --
            ...
            public override void Execute (ScriptableRenderContext context, ref RenderingData rendData) {
                ...
                using (new ProfilingScope (cmd, base.profilingSampler)) {
                    ...
                    cmd.Clear ();
#if UNITY_2022_3_OR_NEWER
                    var desc = new RendererListDesc (_shaderTagId, rendData.cullResults, rendData.cameraData.camera) {
                        sortingCriteria = SortingCriteria.None,
                        renderQueueRange = _queueRange,
                        layerMask = ~0,
                        rendererConfiguration = (PerObjectData) ~0,
...
#else
                    var drawingSettings = CreateDrawingSettings (_shaderTagId, ref rendData, SortingCriteria.None);
                    var filteringSettings = new FilteringSettings (_queueRange, layerMask: ~0);
                    context.DrawRenderers (rendData.cullResults, ref drawingSettings, ref filteringSettings);
#endif
                }
                context.ExecuteCommandBuffer (cmd);
        ...
        internal class WCEEachMeshPass : ScriptableRenderPass {
            ...
            internal void Setup (RenderPassEvent evt, Material mat, LayerMask layerMask, bool useOpaqueTex) {
                ...
                _mat = mat;
                _useOpaqueTex = useOpaqueTex;
                if (useOpaqueTex) ConfigureInput (ScriptableRenderPassInput.Color);
            }
            ...
            public override void RecordRenderGraph (RenderGraph renderGraph, ContextContainer frameData) {
                ...
                using (var builder = renderGraph.AddRasterRenderPass<PassData> (passName, out var passData, profilingSampler)) {
                    ...
                    if (_useOpaqueTex) registerReadTex (builder, resourceDt.cameraOpaqueTexture);
                    // --
                    var desc = new RendererListDesc (_shaderTagIds, rendDt.cullResults, camDt.camera) {
                        sortingCriteria = SortingCriteria.None,
...
#endif // end of UNITY_6000_0_OR_NEWER
            ...
            public override void Execute (ScriptableRenderContext context, ref RenderingData rendData) {
                ...
                using (new ProfilingScope (cmd, base.profilingSampler)) {
                    ...
                    cmd.DrawRendererList (rendererList);
#else
                    var drawingSettings = CreateDrawingSettings (_shaderTagIds, ref rendData, SortingCriteria.None);
                    drawingSettings.overrideMaterial = _mat;
                    drawingSettings.overrideMaterialPassIndex = 0;
                    var filteringSettings = new FilteringSettings (RenderQueueRange.opaque, _layerMask);
                    context.DrawRenderers (rendData.cullResults, ref drawingSettings, ref filteringSettings);
#endif
                }
                context.ExecuteCommandBuffer (cmd);
...
#endif // end of !UNITY_6000_4_OR_NEWER
        }
